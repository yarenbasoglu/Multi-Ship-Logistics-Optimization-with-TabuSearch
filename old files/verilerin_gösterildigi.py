...
def compute_depot_risk(day: int, depot: Depot) -> Optional[str]:
    ...
    projected_level = depot.initial_level - depot.daily_decay * day
    days_to_min = (projected_level - depot.min_cap) / depot.daily_decay
...
def build_customer_jobs(locs: Dict[str, Location], tolerance: int = 2) -> Dict[str, Job]:
    ...
    for loc in locs.values():
        job_id = f"CUST_{loc.loc}"
        jobs[job_id] = Job(
            job_id=job_id,
            job_type="customer",
            location=loc.loc,
            qty=loc.qty,
            release_day=max(1, loc.demand_day - tolerance),
            deadline=min(35, loc.demand_day + tolerance),
            region=loc.region,
        )
    return jobs
...
def build_depot_risk_jobs(day: int, depots: Dict[str, Depot], min_sailing_days: int = 3) -> List[Job]:
    ...
    for depot in depots.values():
        ...
        if projected_level <= threshold:
            risk_jobs.append(
                Job(
                    job_id=f"DEPOT_{depot.depot}_D{day}",
                    job_type="depot",
                    location=depot.depot,
                    qty=max(1, depot.max_cap - projected_level),
                    release_day=day,
                    deadline=min(35, day + min_sailing_days),
                    region=depot.region,
                )
            )
    return risk_jobs
...
def build_daily_critical_jobs(
    ...
    window_customer_jobs = [
        ...
        and not (j.deadline < window_start or j.release_day > window_end)
    ]
    window_customer_jobs.sort(key=lambda j: (j.deadline, -j.qty))
...
def build_job_package(seed_job: Job, candidate_jobs: List[Job]) -> List[Job]:
    ...
    same_region.sort(key=lambda j: (j.deadline, -j.qty))
    if same_region:
...
def run_rolling_horizon_day_loop_step15(
    ...
    """STEP 2-3-4-5: Günlük döngü + pencere + depo risk + kritik iş çıkarma + karar sırası (iş->paket->gemi->takvim).

    Not: Bu fonksiyon şu an "planlama ön-izlemesi" üretir.
    (Paketleme / gemi seçimi / takvime yerleştirme adımlarını Step 6-9 ile tamamlayacağız.)
    """

    # ShipState'leri üret (meşgul günlerden blok aralığı çıkar)
    ship_state_map = build_ship_state_map(ships, busy_days)

    # Depo seviyeleri (simülasyon için)
    ...
    for t in range(1, total_days + 1):
        ...
        # -----------------------
        critical_depot_jobs: List[Job] = []
        depot_risk_snapshot: Dict[str, Dict[str, float]] = {}

        for d_id, depot in depots.items():
            # günlük tüketim
            depot_levels[d_id] = depot_levels[d_id] - max(0, depot.daily_decay)
        ...
        if unassigned_queue:
            ...
            for q in unassigned_queue:
                if q.deadline < window_start:
                    # kaçırıldı: yine de kuyrukta tut (silmek yok)
                    still_waiting.append(q)
                    continue
                if q.deadline > window_end:
                    still_waiting.append(q)
                    continue
                active_jobs.append(q)
                unassigned_ids.discard(q.job_id)
        ...
        # -----------------------
        # STEP 5: Karar sırası (İş -> Paket -> Gemi -> Takvim)
        # Şimdilik sadece "iş seç + paket öner" logluyoruz.
        # -----------------------
        ...
        for job in active_jobs[: min(8, len(active_jobs))]:
            pkg = build_job_package(job, active_jobs)
            ...
            # Step 6-9 gelene kadar yerleştirme yok -> kuyrukta beklet
        ...
        print(
            ...
            f"chosen={len(chosen_jobs)} unassigned_queue={len(unassigned_queue)}"
        )
...
def main():
    ...
    # =========================================================
    # ROLLING HORIZON PREVIEW (STEP 1-5)
    # =========================================================
    ...
    print("\n=== ROLLING HORIZON PREVIEW (Adim 1-5) ===")
    ...
    for rec in plan_logs[:5]:
        critical_depot_count = sum(1 for jid in rec.selected_jobs if jid.startswith('DEPOT_'))
        print(
            f"t={rec.day} window=[{rec.window_start}..{rec.window_end}] "
            f"critical_depot={critical_depot_count} jobs_in_window={len(rec.selected_jobs)} "
            f"queue={rec.unassigned_count}"
        )
        for p in rec.package_previews:
            print(f"    package> {p}")

    print(f"Queue size (end): {len(unassigned_queue)}")
    print("\nEVENT-DRIVEN LOOP PREVIEW FINISHED.\n")
