...
def load_routes(locs: Dict[str, Location]) -> Dict[str, Route]:
    ...
    for r in parse_csv(ROUTES_CSV):
        rid = r["Route"]
        sid = r["Ship"]
        stops = tuple(s.strip() for s in r["Stops"].split("-") if s.strip()) #l5-l6
        ...
        demand_tons = sum(locs[s].qty for s in stops) #rota kapasitesi = uğradığı lokasyonların talep toplamı
...
def generate_depot_routes(
    ...
    for rid, r in routes.items():
        ...
        if r.demand_tons >= ship.capacity: # depo teslimatı için kapasite kalmadı
            continue
        ...
        for d in target_depots: # depo teslimatı ekle
            new_id = f"{rid}_{d.depot}" # yeni rota id'si
            new_stops = r.stops + (d.depot,) # mevcut duraklara depo ekle
            
            new_cost = int(r.cost * 1.2) # depo teslimatı için maliyet %20 artar
            new_duration = r.duration + 1 # depo teslimatı için süre 1 gün artar
            ...
            new_routes[new_id] = Route(
                ...
                is_depot_delivery=True # depo doldurma rotası olarak işaretle müşteri teslimatı değil
            )
            
    # 2. Direkt Depo Rotaları (müşteriye uğramadan direkt depoya giden tedarik rotalarıdır)
...
# ============================================================
# 4) CONSTRAINT CONFIG (Fix included)
# ============================================================
...
class ConstraintConfig:
    ...
    enforce_capacity: bool = True # Gemi kapasitesinden büyük rota kabul edilmez
...
def evaluate(
    ...
    for ship_id, trip_list in sol.items():
        ...
        for rid in trip_list: #rota var mı?
            ...
            total_cost += r.cost + ship.fixed_daily_cost * r.duration # rota maliyeti
        ...
        if cfg.enforce_time_window_pm2: #rota zaman penceresi ±2 gün içinde planlanmalı
    ...
    if cfg.enforce_cover_all_locations: #tüm 24 lokasyon en 1 kere servis edilmeli
    ...
    if cfg.enforce_depot_min_level:
        current_inv = {d: depot.max_cap for d, depot in depots.items()} #başlangıç envanteri max kapasite
        ...
        for sid, sched in ship_schedules.items():
            for rid, start_day in sched.items():
                r = routes[rid]
                if r.is_depot_delivery: #depo teslimatı rotası ise
                    amount = ships[sid].capacity - r.demand_tons #gemi müşteriye teslim ediyor kalan yakıt depoya boşaltılıyor 
                    target_depot = None 
                    for s in r.stops: 
                        if s in depots:
                            target_depot = s
        ...
        for d_id, depot in depots.items():
            ...
            for day in range(1, 36): 
                current_inv[d_id] -= depot.daily_decay # günlük azalma
                daily_supply = deliveries[d_id][day] 
                if daily_supply > 0:
                    current_inv[d_id] += daily_supply # depo teslimatı alındıysa envantere ekle
                    if current_inv[d_id] > depot.max_cap: 
                        current_inv[d_id] = depot.max_cap 
                
                if current_inv[d_id] < depot.min_cap: # depo minimum seviyesinin altına düştü mü?
...
# tek tek kısıtlar kontrol edilerek rota ekleme fonksiyonu
    def can_add_trip(ship_id: str, rid: str) -> bool:
        ...
        #mevcut rota listesi + bu yeni rota süreleri toplamı 35 günü geçerse ekleme
        current = sol[ship_id]
    ...
    for _ in range(400):
        ...
        target_loc = next(iter(uncovered)) #henüz servis edilmemiş bir lokasyon seçilir
        candidates = []

#bu lokasyonu kapsayan ve eklenebilecek rotaları bul
        ...
        top_candidates = candidates[:rcl_size] #en ucuz rcl_size kadar rotayı al 5 tane
        chosen_rid, chosen_sid = random.choice(top_candidates) 
# seçilen rotayı çözüme ekle
...
def tabu_search_multi_trip(
    ...
    tabu_tenure: int = 24, #bir hareket tabu listesine girince kaç iterasyon yasak kalıcak
    seed: int = 11, 
    max_trips_per_ship: int = 9, 
):
    #rasgele seçimlerde tutarlılık için seed belirle
    random.seed(seed)
    by_ship = routes_by_ship(routes)
    ship_ids = list(ships.keys())

# Başlangıç Çözümü - greedy + rastgelelik
    ...
    def decay_tabu(): 
        for k in list(tabu.keys()):
            tabu[k] -= 1 #Her iterasyonda tabu sürelerini 1 azaltıyor. Sıfıra düşünce tabu’dan çıkıyor
            if tabu[k] <= 0: del tabu[k]

    # yeni bir hamleyi tabu listesine ekle
    def add_tabu(sid, action, rid): tabu[(sid, action, rid)] = tabu_tenure 
    def is_tabu(sid, action, rid): return (sid, action, rid) in tabu

    for it in range(1, max_iters + 1): 
        decay_tabu()
        best_cand, best_cand_obj, best_cand_dbg, best_move = None, None, None, None #en iyi aday çözüm ve bilgileri

        for _ in range(neighborhood_size): #240 tane komşu çözüm deniyoruz
            ...
            if action == "add": #gemiye yeni rota ekle
            ...
            elif action == "remove": #gemiden rota çıkar
            ...
            elif action == "swap_order": #gemideki rotaların sırasını değiştir
            ...
            elif action == "shift_ship": #rota başka bir gemiye taşınsın
                if not trips: continue
                rid_from = random.choice(trips) #taşınacak rota
                sid_to = random.choice(ship_ids) #hedef gemi
                ...
                if not candidates_to: continue #eşleşen rota yoksa hamle iptal edilri
                rid_to = candidates_to[0]
                
                # hedef gemide bu rota zaten kullanılıyor mu kontrol et 
                in_use = False
            ...
            if move_info is None:continue # geçersiz hamle atılır
            
            cand_obj, cand_dbg = evaluate(cand, ships, routes, locs, depots, cfg)

            # tabu kontrolü - maliyet mevcut en iyi çözümden kötü ise tabu hamle atlanır
            if is_tabu(*move_info):
                if cand_obj >= best_obj: continue 
            
            # en iyi komşu çözüm güncellenir
...
# en iyi komşu çözüm mevcut çözüm olur -
        current = best_cand
...
#
        ...
        if it % 120 == 0: #her 120 iterasyonda bir ilerleme durumu yazdır
            print(
