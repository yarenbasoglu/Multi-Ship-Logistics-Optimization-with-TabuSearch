...
def compute_depot_risk(day: int, depot: Depot) -> Optional[str]:
    if depot.daily_decay <= 0:# tüketim olmayan depolar riskli değil, onları atla
        return None

    projected_level = depot.initial_level - depot.daily_decay * day # gün sonunda projekte edilen seviye (örneğin, initial=100, decay=10, day=3 -> projected=70)
    days_to_min = (projected_level - depot.min_cap) / depot.daily_decay # min seviyeye kaç gün kaldı? (örneğin, projected=70, min=20, decay=10 -> days_to_min=5)
...
# Adim 3: Depo riskine göre kritik depo işleri oluştur (release=bugün, deadline=5 gün sonrası, qty=depo max cap - projekte seviye)
def build_depot_risk_jobs(day: int, depots: Dict[str, Depot], min_sailing_days: int = 3) -> List[Job]:
    risk_jobs: List[Job] = [] #riskli depolar için oluşturulan işleri eklicez
    for depot in depots.values():
        if depot.daily_decay <= 0: #bu durumda depo stok tüketmiyor riskli değil
            continue
        projected_level = depot.initial_level - depot.daily_decay * max(0, day - 1)
        ...
        if projected_level <= threshold: # depo seviyesi kritik eşiğin altına düşüyor, riskli iş oluştur
            risk_jobs.append( #depo işi oluşturuluyor 
                Job(
                    ...
                    qty=max(1, depot.max_cap - projected_level), # riskli depoya ne kadar malzeme lazım? Projede seviye kritik eşiğin altında, max_cap'e kadar olan farkı qty olarak alabiliriz (örneğin, max_cap=100, projected=70 -> qty=30). Minimum 1 tonluk bir iş oluşturuyoruz, çünkü çok küçük miktarlar paketleme ve atama sürecinde göz ardı edilebilir.
                    release_day=day,
                    deadline=min(35, day + min_sailing_days),  # riskli işin deadline'ı bugün + minimum tedarik süresi (örneğin, bugün=18, min_sailing_days=3 -> deadline=21). Bu, riskli işin planlama sürecinde öncelikli olarak ele alınmasını sağlar, çünkü deadline'ı yaklaşıyor.
                    region=depot.region,
                )
            )
    return risk_jobs
...
def build_job_package(seed_job: Job, candidate_jobs: List[Job]) -> List[Job]:
    ...
    same_region = [
        ...
        and jobs_window_overlap(seed_job, j) #iki işin zaman pencereleri çakışıyor mu
    ]
    ...
    cross_city = [ 
        ...
        if j.job_id != seed_job.job_id
        and j.job_type == "customer"
        and jobs_window_overlap(seed_job, j)
    ]
...
def find_feasible_ship_options_for_package(
    ...
    release_day = max((j.release_day for j in package), default=window_start)
    deadline_day = min((j.deadline for j in package), default=window_end)
    ...
    for r in routes.values():
        ...
        # Backward'e hazırlık olarak pencere içinde uygun bir start var mı bak
        ...
        chosen_start = None
        chosen_end = None
        # Şimdilik "en uygun" = pencere içinde ilk feasible slot (Step 8'de cost ranking)
        for start_day in range(route_earliest_start, route_latest_start + 1):
...
def select_ship_for_package(
    ...
    ranked = sorted(
        feasible_ship_options,
        key=lambda x: (x["end_day"], x["duration"], x["cost"])
    )
    return ranked[0]
...
#yani şu an bu iş şu gemiye sığar mı diye bakar planlama ön izlmesi
    ...
    depot_levels: Dict[str, int] = {
        d_id: (depots[d_id].initial_level if depots[d_id].initial_level > 0 else depots[d_id].max_cap)
...
#her gün yeniden planla, önümüzdeki 10 günü 
    for t in range(1, total_days + 1): # rolling horizon'un her gününde, o günün penceresini belirle, depo riskini hesapla, kritik işleri seç, paketleme ve atama adımlarını uygula, günlük log'u oluştur. Pencere sonunda atanmayan işler unassigned_queue'ya eklenir ve sonraki gün değerlendirilir.
        ...
        for d_id, depot in depots.items(): 
            # günlük tüketim
            depot_levels[d_id] = depot_levels[d_id] - max(0, depot.daily_decay) #günlük tüketimi stoktan düş 

            # geri sayım eşiği: min'e kaç gün kaldı?
            days_to_min = 10**9
            if depot.daily_decay > 0:
                days_to_min = (depot_levels[d_id] - depot.min_cap) / depot.daily_decay
            ...
            # Eşik: 5 gün ve altı kritik (negatifse zaten batmış)
            if days_to_min <= 5:
                critical_depot_jobs.append(
                    Job(
                        ...
                        job_type="depot",
                        location=d_id,
                        qty=0,  # depo işi için qty'yi Step 6-9'da paketleme belirleyecek
                        release_day=t,
                        deadline=min(total_days, t + 5),
                        region=depot.region,
                        meta={"depot_level": depot_levels[d_id], "days_to_min": days_to_min},
                    )
                )
        ...
        # Deadline'a göre sırala (depo işleri zaten başta)
        active_jobs.sort(key=lambda j: (0 if j.job_type == "depot" else 1, j.deadline, j.release_day))
        ...
        # -----------------------
        chosen_jobs = [j.job_id for j in active_jobs[: min(8, len(active_jobs))]]
        ...
        for job in active_jobs[: min(8, len(active_jobs))]:
            pkg = [job] if endgame_direct_mode else build_job_package(job, active_jobs)
            if not pkg:
                step6_no_package += 1
            ...
            if feasible_ship_options:
                ...
                step6_waiting += 1
                # STEP 10: Endgame'de merge paket olmuyorsa tek is direkt atama fallback
                if (
                    endgame_direct_mode
                    and len(pkg) > 1
            ...
            selected_ship_option = select_ship_for_package(
                feasible_ship_options,
                mode=("lowest_cost" if endgame_direct_mode else ship_selection_mode),
            )
        ...
        print(
            ...
            f"critical_depots={len(critical_depot_jobs)} active_jobs={len(active_jobs)} "
            f"chosen={len(chosen_jobs)} queue={len(unassigned_queue)} "
            f"step6_wait={step6_waiting} step7_feasible={step7_feasible_hits} "
            f"step8_selected={step8_selected} step9_assigned={step9_assigned} "
            f"step10_direct={step10_direct_mode_hits}"
        )
        if critical_depot_jobs:
            print("  critical_depot_jobs:", [j.location for j in critical_depot_jobs])
        if chosen_jobs:
            print("  chosen_jobs:", chosen_jobs)
        if day_assignments:
            print("  assignments:", day_assignments[:3])
        # STEP 11: gun bazli rolling horizon log ozeti (hoca icin okunur)
        print(
            f"  rolling_log: t={t} window=[{window_start}..{window_end}] "
            f"critical={len(critical_depot_jobs)} assigned={len(day_assignments)} "
            f"queue={len(unassigned_queue)}"
        )
...
def main():
    ...
    print("\n=== ROLLING HORIZON PREVIEW (Adim 1-12) ===")
    print(f"Toplam customer job: {len(jobs_by_id)}")
    print("Job adedi (musteri bazli):")
    for cid in sorted(locs.keys(), key=lambda x: safe_int(re.sub(r'[^0-9]', '', x), 999)):
        print(f"  {cid} ({humanize_code(cid)}): {job_count_by_customer.get(cid, 0)} job")
        ids = sorted(
            job_ids_by_customer.get(cid, []),
            key=lambda x: safe_int(re.search(r"_D(\\d+)$", x).group(1), 999) if re.search(r"_D(\\d+)$", x) else 999
        )
        if ids:
    ...
    for rec in plan_logs[:5]:
        chosen = rec.get("chosen_jobs", [])
        window = rec.get("window", (None, None))
        critical_depot_count = sum(1 for jid in chosen if str(jid).startswith("DEPOT_"))
        print(
            f"t={rec.get('day')} window=[{window[0]}..{window[1]}] "
            f"mode={rec.get('mode')} "
            f"critical_depot={critical_depot_count} chosen={len(chosen)} "
            f"queue={rec.get('unassigned_queue_size')} "
            f"s6_wait={rec.get('step6_waiting')} s7_ok={rec.get('step7_feasible_hits')} "
            f"s8_sel={rec.get('step8_selected')} s9_assigned={rec.get('step9_assigned')} "
            f"s10_direct={rec.get('step10_direct_mode_hits')}"
        )
        for p in rec.get("packages_preview", []):
            print(f"    package> {p}")
    ...
    print("\n[3] Servis Edilen Isler (musteri -> planlanan gun)")
    ...
    if served_records:
        served_records.sort(key=lambda r: (safe_int(r["planned_day"], 999), str(r["job_id"])))
        for r in served_records:
            loc_name = humanize_code(r["location"]) if r["location"] else r["location"]
            print(
                f"  {r['job_id']} ({loc_name}) -> start={r['planned_day']} end={r['end_day']} "
    ...
    if depot_ids_sorted:
        for d_id in depot_ids_sorted:
            ...
            for rec in plan_logs:
                ...
                if level is None:
                    continue
                levels.append(f"D{rec.get('day')}={int(round(level))}")
                if minv is not None and level < minv:
                    breaches.append(rec.get("day"))
                if minv is not None:
                    mins.append(str(int(round(minv))))
            min_text = mins[0] if mins else "?"
            print(f"  {d_id} (min={min_text})")
            if levels:
                print("   ", " | ".join(levels[:20]))
                if len(levels) > 20:
                    print("   ", " | ".join(levels[20:]))
            print(f"    min_alti_gunler={breaches if breaches else 'yok'}")

    print(f"\nFinal unassigned_queue size: {len(unassigned_queue)}")
    return  # Şimdilik Tabu çalışmasın diye koyduk
